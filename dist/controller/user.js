...
var userInfo = exports.userInfo = async function userInfo(req, res) {
    try {
        var users = await _user2.default.findAll({ attributes: ['identifier', 'username', 'name', 'phoneNumber', 'createdAt', 'updatedAt', 'password'] });
        res.json({
            ok: true,
            message: "",
            users: users
...
var newAccount = exports.newAccount = function newAccount(req, res) {
    ...
    _user2.default.findOne({ where: { username: username } }).then(function (user) {
        ...
        if (user != null) {
            ...
            _user2.default.findOrCreate({ where: { username: username, password: password, name: name, phoneNumber: phoneNumber } }).then(async function (_ref) {
